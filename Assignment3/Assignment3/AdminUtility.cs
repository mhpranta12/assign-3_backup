...
namespace Assignment3
{
    internal class AdminUtility
    {
        ...
        // Create a Course
        public void CreateCourse(string course_name, string course_code, double fees,string start_date, string end_date)
        {
            DateTime startdate = GetDate(start_date);
            DateTime enddate = GetDate(end_date);
            context.Courses.Add(new Course { Name = course_name, CourseCode = course_code, Fees = 2000,StartDate=startdate ,  EndDate = enddate });
            if (context.SaveChanges() > 0)
            {
